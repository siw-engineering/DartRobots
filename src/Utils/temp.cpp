...
Terrain TerrainGenerator::generateSteps(const TerrainConfig &config)
{
    ...
    double r = 0;
    for (int64_t i = 0; i < nVerticesX; ++i)
    {
        r = 0;
        for (int64_t j = 0; j < nVerticesY; ++j)
        {
            // Add slope to height
            auto h = r * sin(config.slope * (3.1415 / 180));
            hmap.row(i)(j) += h;
            r += config.resolution;
        }
    }
    ...
    {
        hmap.transpose();
        for (auto i = 0; i < nVerticesX; i++)
        {
            for (auto j = 0; j < nVerticesY; j++)
            {
                terrain.heights.emplace_back(hmap.row(i)(j));
            }
        }
...
//
// Created by developer on 7/18/22.
//
